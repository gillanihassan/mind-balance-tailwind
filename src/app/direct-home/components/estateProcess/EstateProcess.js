...
export default function EstateProcess() {
  return (
    <div className="max-w-[1400px] mx-auto bg-[#e3f3f3] px-6 md:px-16 py-18 mt-10">
      <div className="max-w-screen-2xl mx-auto flex flex-col md:flex-row">
        ...
        <div className="md:w-5/12 order-1 md:order-2 mt-10 md:mt-10">
          <h2 className="text-[#004f59] font-bold text-[35px] font-inter max-w-[365px]">
            ...
            {estateContent.checklist.map((item, index) => (
              <div className="flex" key={index}>
                <Image
                  ...
                  alt="check icon"
                />
                <p className="text-[#808080] font-inter font-normal text-[28px] leading-[28px] ml-3">
                  {item}
                </p>
              </div>
            ))}
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            {estateContent.buttons.map((btn, index) => (
              <button
                key={index}
                className={`${btn.bg} ${btn.text} ${btn.border} font-inter font-bold text-[16px] px-9 py-3 rounded`}
              >
                {btn.label}
              </button>
            ))}
